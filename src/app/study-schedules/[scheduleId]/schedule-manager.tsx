...
export function ScheduleManager({ schedule, courseName, deckName, cards, selectedCardIds, dates, resumeAnytime }: {
    ...
    return (
      <form onSubmit={save} className="surface-panel mt-5 space-y-7">
        ...
        <section className="space-y-4 border-t border-border pt-6" aria-labelledby="edit-cards-heading">
          ...
          <fieldset disabled={busy} className="space-y-2"><legend className="sr-only">Card selection method</legend>{([ ["all", "Entire deck"], ["starred", "Starred cards"], ["manual", "Manual selection"] ] as const).map(([value, label]) => <label key={value} className="mode-option flex min-h-11 cursor-pointer items-center gap-3 rounded-lg border border-border px-3 py-2"><input type="radio" name="edit-card-mode" checked={mode === value} onChange={() => setMode(value)} className="accent-primary" /><span className="font-medium">{label}</span></label>)}</fieldset>
  ...
  return (
    <>
      ...
      <dl className="mt-6 flex flex-wrap gap-x-10 gap-y-3 border-y border-border py-4"><div><dt className="text-xs font-medium text-muted-foreground">Selected cards</dt><dd className="mt-1 text-sm font-semibold tabular-nums">{selectedCardIds.length}</dd></div><div><dt className="text-xs font-medium text-muted-foreground">Exam date</dt><dd className="mt-1 text-sm font-semibold">{schedule.examDate ? formatCalendarDate(schedule.examDate) : "No exam date"}</dd></div></dl>
      ...
      <section className="mt-8" aria-labelledby="schedule-review-dates"><h2 id="schedule-review-dates" className="section-title">Review dates</h2><div className="mt-3 grid gap-4 sm:grid-cols-2"><div><h3 className="text-sm font-semibold text-foreground">To study</h3>{incompleteDates.length ? <ul className="mt-2 space-y-2">{incompleteDates.map((date) => <li key={date.id} className="flex flex-col gap-2 rounded-lg border border-border bg-white px-4 py-3 text-sm sm:items-start"><span className="flex flex-wrap items-center gap-2 font-medium">{formatCalendarDate(date.review_date)}<span className={`schedule-status ${today && date.review_date < today ? "schedule-status--overdue" : ""}`}>{today ? date.review_date < today ? "Overdue" : date.review_date === today ? "Today" : "Upcoming" : "Upcoming"}</span></span><Button type="button" variant="secondary" disabled={busy || Boolean(starting) || (selectedCards.length === 0 && !date.resumable)} onClick={() => void startStudy(date.id)} className="w-full sm:w-auto">{starting === date.id ? "Starting…" : date.resumable ? "Resume review" : "Study this review"}</Button></li>)}</ul> : <p className="field-hint mt-2">No incomplete dates.</p>}</div><div><h3 className="text-sm font-semibold text-foreground">Completed</h3>{completedDates.length ? <ul className="mt-2 space-y-2">{completedDates.map((date) => <li key={date.id} className="rounded-lg border border-border bg-white px-4 py-3 text-sm text-muted-foreground">{formatCalendarDate(date.review_date)} · Completed</li>)}</ul> : <p className="field-hint mt-2">No completed dates yet.</p>}</div></div></section>
