...
export function DeckManager({ courseId, courseName, decks }: { courseId: string; courseName: string; decks: Deck[] }) {
  ...
  return <>
    ...
    {decks.length === 0 ? <div className="empty-panel mt-5"><h3 className="empty-panel__title">No decks yet</h3><p className="empty-panel__copy">Add a deck to start collecting cards for this course.</p></div> :
      <div className="mt-5 grid gap-3 sm:grid-cols-2">{decks.map((deck) => <Card key={deck.id} className="min-w-0 py-0 transition-colors hover:border-[#c9a1b1]"><CardContent className="flex items-start gap-2 p-5"><Link href={`/courses/${courseId}/decks/${deck.id}`} className="min-w-0 flex-1 rounded-sm focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring"><h2 className="break-words text-base font-semibold text-foreground hover:text-primary">{deck.name}</h2>{deck.description && <p className="mt-2 line-clamp-2 whitespace-pre-wrap break-words text-sm text-muted-foreground">{deck.description}</p>}</Link><DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon" aria-label={`Manage ${deck.name}`} className="-mr-2 -mt-2 text-muted-foreground" />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-40"><DropdownMenuItem onClick={() => startEditing(deck)}><Pencil /> Edit deck</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => setDeleting(deck)}><Trash2 /> Delete</DropdownMenuItem></DropdownMenuContent></DropdownMenu></CardContent></Card>)}</div>}
